...
function validateGhRepository(ghRepository: string): void {
    const expectedHost = new URL(process.env['GITHUB_SERVER_URL'] ?? 'https://github.com').hostname;
    let repoHost: string;
    try {
        repoHost = new URL(`https://${ghRepository}`).hostname;
    } catch {
        throw new Error(`'gh-repository' has an invalid format: '${ghRepository}'`);
    }
    if (repoHost !== expectedHost) {
        throw new Error(
            `'gh-repository' must point to the configured GitHub server '${expectedHost}', got '${repoHost}'`,
        );
    }
}
...
export async function configFromJobInput(): Promise<Config> {
    ...
    if (ghRepository) {
        ...
        validateGhRepository(ghRepository);
    }
